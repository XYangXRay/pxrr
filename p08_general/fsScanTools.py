...
from p08_general import fio_reader
...
class BaseScan(object):
    ...
    def __init__(self):
        ...
        self.detector_config = self.__class__.DETECTOR_CONFIG.copy()
        self.detector_states = {}
        for detector_name in self.detector_config:
            self.detector_states[detector_name] = False
            setattr(self, "is_%s" % detector_name, False)
    ...
    def load_scan(self, filename = None, auto_load_images = True, auto_remove_double = False):
        ...
        if not self.scan_cmd.find('lscan') == -1 and self.scan_cmd.find('hklscan') == -1:
            ...
            self.scan_motors = [ numpy.array(data["kozhue6cctrl_h"]), numpy.array(data["kozhue6cctrl_k"]), numpy.array(data["kozhue6cctrl_l"]) ]
        #elif not self.scan_cmd.find('quickascan') == -1 or not self.scan_cmd.find('quickdscan') == -1:
        #    mot1_name = self.scan_cmd.split()[1]
        #    self.scan_motor_names = [mot1_name]
        #    #self.scan_motors = [ numpy.array(data['position']) ] 
        #    self.scan_motor_names = [mot1_name]
        #    self.scan_motors = [ numpy.array(data[mot1_name]) ]
        #    self.quickscan = True
        elif not self.scan_cmd.find('burst') == -1:
            self.burstscan = True
        elif not self.scan_cmd.find('ascan') == -1 or not self.scan_cmd.find('dscan') == -1:
            mot1_name = self.scan_cmd.split()[1]
            self.scan_motor_names = [mot1_name]
            self.scan_motors = [ numpy.array(data[mot1_name]) ]   
            if 'quick' in self.scan_cmd:
                self.quickscan = True
            ...
            self.scan_motors = [ numpy.array(data[mot1_name]), numpy.array(data[mot2_name]), numpy.array(data[mot3_name]) ]
        ...
        self._sync_detector_attributes()
        ...
        if auto_load_images:
            ...
            if self.quickscan:
                for detector in self.image_data.keys():                   
                    try:
                    #if True:
                        detector_rois = self.rois[detector]
                        res = ScanAnalyzer.extract_rois(self, detector_rois)
                        
                        for key in res[detector]:
                            self.data[detector + '_' + key] = res[detector][key]
                    except:
                        pass
    ...
    def load_image_stack(self):
        ...
        for detector_name in self.detectors:
            ...
            if isinstance(detector_data, numpy.ndarray) and detector_data.ndim == 3:
                detector_data = self._remove_double_fromstack(detector_data)

            self.image_data[detector_name] = detector_data

    def _detect_detectors(self, column_names):
        for col_name in column_names:
            col_name_lower = col_name.lower()
            for detector_name, config in self.detector_config.items():
                if self._matches_detector_column(col_name_lower, config):
                    self.detector_states[detector_name] = True

    def _detect_detectors_from_header(self):
        signalcounter = str(self.header_info.get("signalcounter", "")).lower()
        rois = self.header_info.get("rois", {})
        roi_names = [str(key).lower() for key in rois.keys()]

        for detector_name, config in self.detector_config.items():
            identify_patterns = [pattern.lower() for pattern in config.get("identify_columns", [])]
            for pattern in identify_patterns:
                if pattern in signalcounter or pattern in roi_names:
                    self.detector_states[detector_name] = True
                    break

    def _matches_detector_column(self, col_name, config):
        include_patterns = [pattern.lower() for pattern in config.get("identify_columns", [])]
        exclude_patterns = [pattern.lower() for pattern in config.get("exclude_columns", [])]

        if any(pattern in col_name for pattern in exclude_patterns):
            return False

        return any(pattern in col_name for pattern in include_patterns)

    def _sync_detector_attributes(self):
        self.detectors = []
        for detector_name in self.detector_config:
            state = bool(self.detector_states.get(detector_name, False))
            setattr(self, "is_%s" % detector_name, state)
            if state:
                self.detectors.append(detector_name)

    def _load_detector(self, detector_name):
        if detector_name not in self.detector_config:
    ...
    def _load_eiger(self, detector):
        ...
        #print( list(nxsfile['/entry/data/'].keys()) )
        nxsdata = None        

        # check all data entries and merge into a single array.
        # in all my tests the key list was sorted. not sur if
        # one should add a sorting here to make sure...
        for val in list(nxsfile['/entry/data/'].keys()):
            #print(val)
            data = nxsfile.get('/entry/data/%s' % val)
            #print(type(data))
            ...
            if nxsdata is None:
                ...
                #nxsdata = numpy.concatenate((nxsdata, data.value) )
                nxsdata = numpy.concatenate((nxsdata, data[()]) )
                #print( nxsdata.shape )

        # check for extra empty frames:
        empty_index = None
        for idx in range(nxsdata.shape[0]):
            if nxsdata[idx,:,:].sum().sum() == 0:
                empty_index = idx
                break
        if not empty_index is None:
            print('found empty frame at index: %d. Skip all frames after' % empty_index)
            nxsdata = nxsdata[:empty_index-1,:,:]
    ...
    def _load_lambda(self, detector):
        '''
    ...
    def _read_2d_image_file(self, filename):
        ext = os.path.splitext(filename)[1].lower()
        if ext in ('.tif', '.tiff') and tifffile is not None:
            return tifffile.imread(filename)
        return fabio.open(filename).data

    def _load_ordered_2d_series(self, image_dir, detector, index_fmt, file_extensions=('.tif', '.cbf'),
                                exclude_substrings=None, max_workers=None):
        exclude_substrings = tuple(exclude_substrings or [])

        all_files = []
        for ext in file_extensions:
            all_files.extend(glob.glob('%s%s*%s' % (image_dir, self.scan_name, ext)))

        all_files = sorted([item.replace('\\', '/') for item in all_files])
        if exclude_substrings:
            all_files = [item for item in all_files if not any(excl in item for excl in exclude_substrings)]

        if len(all_files) == 0:
            print("no image files found for %s in %s" % (detector, image_dir))
            return None

        file_map = {}
        for filename in all_files:
            base, ext = os.path.splitext(filename)
            file_map[base] = filename

        try:
            detector_idx = self.data["%s_index" % detector]
        except:
            print("no index column for %s available. use number of points " % detector)
            detector_idx = range(len(self.scan_motors[0]))

        ordered_files = []
        for idx in detector_idx:
            base_name = "%s%s%s" % (image_dir, self.scan_name, index_fmt % idx)
            if base_name in file_map:
                ordered_files.append(file_map[base_name])

        if len(ordered_files) == 0:
            print("no matching indexed image files found for %s in %s" % (detector, image_dir))
            return None

        if max_workers is None:
            max_workers = min(8, max(1, len(ordered_files)))

        if max_workers <= 1:
            images = [self._read_2d_image_file(filename) for filename in ordered_files]
        else:
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                images = list(executor.map(self._read_2d_image_file, ordered_files))

        return numpy.stack(images, axis=0)
    ...
    def _load_pilatus(self, detector):
        ...
        subfolder = self.DETECTOR_CONFIG[detector]["subfolder"]
        self.pilatus_dir = "%s/%s/%s/" % (self.scan_dir, self.scan_name, subfolder)
        return self._load_ordered_2d_series(self.pilatus_dir, detector, "_%05d")

    def _load_pe(self, detector):
        '''
        load tif or cbf files from a PerkinElmer detector and return a numpy 3D array with the data.

        If an index column for the detector is used, only files with corresponding index are loaded,
        i.e. data index and image index are matching.
        '''

        subfolder = self.DETECTOR_CONFIG[detector]["subfolder"]
        self.pe_dir = "%s/%s/%s/" % (self.scan_dir, self.scan_name, subfolder)
        return self._load_ordered_2d_series(
            self.pe_dir,
            detector,
            "-%05d",
            exclude_substrings=['.raw', '.dark']
        )
    ...
    def _load_mythen(self, detector):
        ...
        subfolder = self.DETECTOR_CONFIG[detector]["subfolder"]
        self.mythen_dir = "%s/%s/%s/" % (self.scan_dir, self.scan_name, subfolder)
    ...
    def _load_amptek(self):
        '''
        load *mca_s*.fio files from mythen detector
        '''

        self.amptek_dir = "%s/%s/" % (self.scan_dir, self.scan_name)

        file_list = glob.glob("%s%s_mca_s*.fio" % (self.amptek_dir, self.scan_name) )
        file_list = sorted(file_list)
        
        first_file = numpy.array(fio_reader.read(file_list[0])[2]['amptek_spectrum'])
        ...
        cnt = 0
        for filename in sorted(file_list):
            image[cnt,:] = numpy.array(fio_reader.read(filename)[2]['amptek_spectrum'])
            cnt += 1
    ...
    def correct_absorber(self):
    
        pos = self.scan_motors[0]
        
        atten = self.data["atten_position"]
        
        img_key = list(self.image_data.keys())[0]
        
        images = self.image_data[img_key].sum(1).sum(1)
        ...
        for idx in range(len(pos)):
            ...
            if idx > 0:
                if numpy.abs(pos[idx]-last_pos) == 0:
                    change_idxes.append(idx-1)
                    
                    
                    
                    print( "atten change: %6.2g to %6.2g" % (atten[idx-1], atten[idx]))
                    print( " factor: %6.2g vs. %6.2g" % (atten[idx-1]/atten[idx], images[idx]/images[idx-1]))
                    
                    change_val.append(images[idx]/images[idx-1])

            last_pos = pos[idx]
            
        new_atten = numpy.ones(len(atten))
        
        for idx in range(len(change_idxes)):
            
            change_idx = change_idxes[idx]
            
            new_atten[0:change_idx+1] *= change_val[idx]
            
        #print(self.data.keys())
        #plt.semilogy(new_atten*numpy.array(self.data['p100k_roi1']))
        #plt.semilogy(atten*numpy.array(self.data['p100k_roi1']))
        #plt.show()
            
        self.data["atten_position"] = new_atten
            ...
            if len(img_stack.shape) == 3:
               # print("2D detector image stack")
                ...
                for idx in range(img_nr):
                    ...
                    for key in rois:
                        roi_img = img[int(rois[key][1]):int(rois[key][3]), int(rois[key][0]):int(rois[key][2])]
...
class ScanFitter(object):
    ...
    @staticmethod
    def gaussLorentzianFit(scan, detector, roi, peaks, wichtung_params = None, export = None):
        ...
        if not roi is None:
            ...
            roi_intensity = scan.data[detector]
        ...
        errfunc = lambda p, x, y: wichtung(wichtung_params,x)*numpy.abs( numpy.log10(fitfunc(p, x)+1) - numpy.log10(y+1) ) # Distance to the target function
        ...
        #ydata = {'data' : roi_intensity, 'fit' : ScanFitter._gauss_lorentzian(xcol, p1), 'start' : ScanFitter._gauss_lorentzian(xcol, params)  }
        ...
        if not export is None:
            ...
            for idx in range(len(peak_list)):
                ...
                fid.write("# % d: %s\n" % (idx, name))
                fid.write("# type: " + peaks[name]["type"] + "\n")
                
                if peaks[name]["type"] == "gauss":
                    params = p1[3:6]           
                    params = numpy.append(params, p1[-1])
                    fid.write("# params: " + repr(params) + "\n")
                    result = ScanFitter._gauss(xcol, params)
                elif peaks[name]["type"] == "lorentzian":
                    params = p1[0:3]           
                    params = numpy.append(params, p1[-1])
            ...
            for idx in range(len(xcol)):
                fid.write("%20.5f " % xcol[idx])
                for name in col_names:
                    fid.write("%20.5f " % ydata[name][idx])
                fid.write("\n")
    ...
    def multiVoigtFit(scan, detector, roi, peaks):
        ...
        ydata = {'data' : roi_intensity, 'before_fit' : ScanFitter._multi_voigt(xcol, *param_tuple ), 'log_fit' : ScanFitter._multi_voigt(xcol, *p2)  }
        #ydata = {'data' : roi_intensity, 'before_fit' : ScanFitter._multi_gauss(xcol, params) }
    ...
    def multiFit(scan, detector, roi, peaks):
        ...
        fitfunc = lambda p, x: ScanFitter._multi_gauss(x, p) # Target function
        errfunc = lambda p, x, y: numpy.sum(numpy.abs(numpy.log10(fitfunc(p, x)) - numpy.log10(y))) # Distance to the target function
    ...
    def _gauss(x, params): 
        ...
        intensity = params[2] / (numpy.sqrt(2*numpy.pi*params[0]**2)) * numpy.exp(-0.5 * ( (numpy.array(x)-params[1])/params[0])**2 ) + params[3]
...
class ScanPlot(object):
    ...
    def plotRois(scan, detector, rois, xcolName=None):
        ...
        if "background" in roi_intensity:
            ...
            background_area = (rois["background"][2]-rois["background"][0])*(rois["background"][3]-rois["background"][1])
    ...
    @staticmethod
    def export2ascii(scan, detector, rois, filename, roi_key='roi1', xcolName=None, bck=None):
        ...
        data = open(filename, 'w')
        # write header
        data.write("# %s\n" % (scan.header_info['scan_cmd']))
        data.write("# om  = %f\n" % (scan.motor_positions['om']))
        data.write("# omh  = %f\n" % (scan.motor_positions['omh']))
        data.write("# phis = %f\n" % (scan.motor_positions['phis']))
        data.write("# chi  = %f\n" % (scan.motor_positions['chi']))
        data.write("# tt = %f\n" % (scan.motor_positions['tt']))
        data.write("# tth = %f\n" % (scan.motor_positions['tth']))
        data.write("# atten = %f\n" % (scan.motor_positions['atten']))
        data.write("# energyfmb = %f\n" % (scan.motor_positions['energyfmb']))
        for pos, inten in zip(xcol, roi_intensity[roi_key]):
            #print("%6.4f, %6.4f" % (pos, inten-bck_inten))
            data.write("%6.4f %f\n" % (pos, inten))
        data.close()
        print('roi: %s exported to %s' % (roi_key, filename))
...
if __name__ == '__main__':
    ...
    '''
    filenames = ['/asap3/petra3/gpfs/p08/2019/data/11006183/raw/nfosto190211_00286.fio', '/asap3/petra3/gpfs/p08/2019/data/11006183/raw/nfosto190211_00287.fio']
    rois = {"roi1" : [761, 279, 812, 290], "roi2" : [761, 279, 1220, 290], "roi3" : [940, 279, 1220, 290], "background" : [1220, 279, 1340, 290] }
    ...
    #ScanFitter.gaussLorentzianFit(combined_scan, "lambda", rois["roi2"], {"film" : {"type" : "gauss", "params" : [0.015,0.95,2e4,4e4]}, "substrate" :  {"type" : "lorentzian", "params" : [1.01,0.003,1e3,1.2e5]} })
    ...
    #filename = '/asap3/petra3/gpfs/p08/2025/data/11021672/raw/lasmo1_p10_s3_05795.fio'
    #
    #rois = {"roi1" : [163, 71, 174, 76], "roi2" : [164, 69, 174, 79], "roi3" : [0, 0, 487, 194] }
    #scan1 = BaseScan()
    #scan1.load_scan(filename)
    ...
    filename = '/asap3/petra3/gpfs/p08/2025/data/11021928/raw/mbe060_00047.fio'
